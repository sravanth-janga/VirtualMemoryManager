...
// Multi-Level(2 levels) page table with page size 1kb
// RAM = 16MB = 2^24 bytes
// VAS = 20bits
// PFN = 14bits
// VPN = 10 bits
// No.of pages = (2^20/(2^10)) = 2^10
// We divide page table across multiple pages and refer them through a pagedirectory
// Total Size of pageTable = (2^10 * (size of pte)) = (2^10 *(4))
// No.of pde = (2^12/2^10) = 2^2 = 4
// VPN:PDE = 2-bits ; PTindex = 10-2 = 8-bits.
/*
Assumptions:
 -> only 512 pages are valid out of 2^10 pages.
*/

/*
    Control Flow to get pagetable entry: (if tlb miss)
    1) use first 2 bits(most significant) to locate the pde in the page dir
    2) use next 8 bits to identify the page table entry.
...
char* get_binary(void* n,int l){
    ...
    // check page directory and then pte;
...
pte_t get_pte(vpn_t vpn){
    uint8_t pdi = (vpn&255)>>8;
    if (PageDir[pdi].valid==0){
        puts("pages are not allocated!");
        _exit(0);
    }
    int pti = (vpn&768)>>2;
    return *(PageDir[pdi].PFN+pti);
}
uint8_t get_pde_index(vaddr_t va){
    return (va&PD_MASK)>>PD_SHIFT;
}
vpn_t get_vpn(vaddr_t virtualAddress){
    if(virtualAddress >=pow(2,20)){
...
void AssignPFN(){
    ...
    for(int i= 0;i<2;i++){
        PageDir[i].valid = 1;
        pte_t* PT =  (pte_t*)malloc(256*sizeof(pte_t));;
        for (int j=0;j<256;j++){
            (PT+j)->PFN = rand() % (int)(pow(2,24));
            (PT+j)->KernelMode = 0;
        // present and protection bits.
            (PT+j)->Present = 1;
            (PT+j)->Protection_bits.read = 1;
            (PT+j)->Protection_bits.write = 1;
            (PT+j)->Protection_bits.execute = 1;
            (PT+j)->Valid = 1;
